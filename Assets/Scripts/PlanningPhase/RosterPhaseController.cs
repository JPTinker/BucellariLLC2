...
public class RosterPhaseController : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        WarnIfMissing(_navDecision, "nav-decision");
        ...
        // are visible but disabled until those screens exist.
        if (_navDecision != null) _navDecision.clicked += OnNavDecisionClicked;
    ...
    /// <summary>
    /// Hands off to the Decision tab. PhaseTabSwitcher owns both screens'
    /// UIDocuments and just toggles which one is visible - it doesn't touch
    /// either screen's internal state, so mid-animation coroutines here
    /// (reveal/level-up/draft) are still running underneath and pick back up
    /// exactly where they left off if the player switches back to Roster.
    /// </summary>
    private void OnNavDecisionClicked()
    {
        PhaseTabSwitcher.Instance?.ShowDecision();
    }
